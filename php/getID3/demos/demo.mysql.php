...
if (!isset($ExistingTableFields['encoder_options'])) { // Added in 1.7.0b2
	...
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` ADD `encoder_options` VARCHAR(255) default "" NOT NULL AFTER `error`');
	mysqli_query_safe('OPTIMIZE TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`');
}
if (isset($ExistingTableFields['track']) && ($ExistingTableFields['track']['Type'] != 'varchar(7)')) { // Changed in 1.7.0b2
	...
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` CHANGE `track` `track` VARCHAR(7) default "" NOT NULL');
	mysqli_query_safe('OPTIMIZE TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`');
}
if (!isset($ExistingTableFields['track_volume'])) { // Added in 1.7.0b5
	...
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` ADD `track_volume` FLOAT NOT NULL AFTER `error`');
	mysqli_query_safe('OPTIMIZE TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`');
}
if (!isset($ExistingTableFields['remix'])) { // Added in 1.7.3b1
	echo '<b>adding field `encoder_options`, `alternate_name`, `parody`</b><br>';
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` ADD `remix` VARCHAR(255) default "" NOT NULL AFTER `title`');
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` ADD `alternate_name` VARCHAR(255) default "" NOT NULL AFTER `track`');
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` ADD `parody` VARCHAR(255) default "" NOT NULL AFTER `alternate_name`');
	mysqli_query_safe('OPTIMIZE TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`');
}
if (isset($ExistingTableFields['comments_all']) && ($ExistingTableFields['comments_all']['Type'] != 'longtext')) { // Changed in 1.9.0
	...
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` CHANGE `comments_all`     `comments_all`     LONGTEXT NOT NULL');
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` CHANGE `comments_id3v2`   `comments_id3v2`   LONGTEXT NOT NULL');
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` CHANGE `comments_ape`     `comments_ape`     LONGTEXT NOT NULL');
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` CHANGE `comments_lyrics3` `comments_lyrics3` LONGTEXT NOT NULL');
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` CHANGE `warning`          `warning`          LONGTEXT NOT NULL');
	mysqli_query_safe('ALTER TABLE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` CHANGE `error`            `error`            LONGTEXT NOT NULL');
}
...
if (!empty($_REQUEST['scan']) || !empty($_REQUEST['newscan']) || !empty($_REQUEST['rescanerrors'])) {
	...
	foreach ($FilesInDir as $filename) {
		...
		if (empty($ThisFileInfo['fileformat'])) {
			...
			if (!empty($_REQUEST['rescanerrors'])) {
				...
				$SQLquery .= '  `last_modified` = "'.   mysqli_real_escape_string($link, !empty($ThisFileInfo['file_modified_time']            ) ?                          $ThisFileInfo['file_modified_time']              : '').'"';
				$SQLquery .= ', `md5_file` = "'.        mysqli_real_escape_string($link, !empty($ThisFileInfo['md5_file']                      ) ?                          $ThisFileInfo['md5_file']                        : '').'"';
				$SQLquery .= ', `md5_data` = "'.        mysqli_real_escape_string($link, !empty($ThisFileInfo['md5_data']                      ) ?                          $ThisFileInfo['md5_data']                        : '').'"';
				$SQLquery .= ', `md5_data_source` = "'. mysqli_real_escape_string($link, !empty($ThisFileInfo['md5_data_source']               ) ?                          $ThisFileInfo['md5_data_source']                 : '').'"';
				$SQLquery .= ', `filesize` = "'.        mysqli_real_escape_string($link, !empty($ThisFileInfo['filesize']                      ) ?                          $ThisFileInfo['filesize']                        :  0).'"';
				$SQLquery .= ', `fileformat` = "'.      mysqli_real_escape_string($link, !empty($ThisFileInfo['fileformat']                    ) ?                          $ThisFileInfo['fileformat']                      : '').'"';
				$SQLquery .= ', `audio_dataformat` = "'.mysqli_real_escape_string($link, !empty($ThisFileInfo['audio']['dataformat']           ) ?                          $ThisFileInfo['audio']['dataformat']             : '').'"';
				$SQLquery .= ', `video_dataformat` = "'.mysqli_real_escape_string($link, !empty($ThisFileInfo['video']['dataformat']           ) ?                          $ThisFileInfo['video']['dataformat']             : '').'"';
				$SQLquery .= ', `vbr_method` = "'.      mysqli_real_escape_string($link, !empty($ThisFileInfo['mpeg']['audio']['VBR_method']   ) ?                          $ThisFileInfo['mpeg']['audio']['VBR_method']     : '').'"';
				$SQLquery .= ', `audio_bitrate` = "'.   mysqli_real_escape_string($link, !empty($ThisFileInfo['audio']['bitrate']              ) ?                 floatval($ThisFileInfo['audio']['bitrate'])               :  0).'"';
				$SQLquery .= ', `video_bitrate` = "'.   mysqli_real_escape_string($link, !empty($ThisFileInfo['video']['bitrate']              ) ?                 floatval($ThisFileInfo['video']['bitrate'])               :  0).'"';
				$SQLquery .= ', `playtime_seconds` = "'.mysqli_real_escape_string($link, !empty($ThisFileInfo['playtime_seconds']              ) ?                 floatval($ThisFileInfo['playtime_seconds'])               :  0).'"';
				$SQLquery .= ', `track_volume` = "'.    mysqli_real_escape_string($link, !empty($ThisFileInfo['replay_gain']['track']['volume']) ?                 floatval($ThisFileInfo['replay_gain']['track']['volume']) :  0).'"';
				$SQLquery .= ', `comments_all` = "'.    mysqli_real_escape_string($link, !empty($ThisFileInfo['comments']                      ) ?                serialize($ThisFileInfo['comments'])                       : '').'"';
				$SQLquery .= ', `comments_id3v2` = "'.  mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']['id3v2']                 ) ?                serialize($ThisFileInfo['tags']['id3v2'])                  : '').'"';
				$SQLquery .= ', `comments_ape` = "'.    mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']['ape']                   ) ?                serialize($ThisFileInfo['tags']['ape'])                    : '').'"';
				$SQLquery .= ', `comments_lyrics3` = "'.mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']['lyrics3']               ) ?                serialize($ThisFileInfo['tags']['lyrics3'])                : '').'"';
				$SQLquery .= ', `comments_id3v1` = "'.  mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']['id3v1']                 ) ?                serialize($ThisFileInfo['tags']['id3v1'])                  : '').'"';
				$SQLquery .= ', `warning` = "'.         mysqli_real_escape_string($link, !empty($ThisFileInfo['warning']                       ) ?            implode("\t", $ThisFileInfo['warning'])                        : '').'"';
				$SQLquery .= ', `error` = "'.           mysqli_real_escape_string($link, !empty($ThisFileInfo['error']                         ) ?            implode("\t", $ThisFileInfo['error'])                          : '').'"';
				$SQLquery .= ', `album` = "'.           mysqli_real_escape_string($link, !empty($ThisFileInfo['comments']['album']             ) ?            implode("\t", $ThisFileInfo['comments']['album'])              : '').'"';
				$SQLquery .= ', `genre` = "'.           mysqli_real_escape_string($link, !empty($ThisFileInfo['comments']['genre']             ) ?            implode("\t", $ThisFileInfo['comments']['genre'])              : '').'"';
				$SQLquery .= ', `comment` = "'.         mysqli_real_escape_string($link, !empty($ThisFileInfo['comments']['comment']           ) ?            implode("\t", $ThisFileInfo['comments']['comment'])            : '').'"';
				$SQLquery .= ', `artist` = "'.          mysqli_real_escape_string($link, !empty($ThisFileInfo['comments']['artist']            ) ?            implode("\t", $ThisFileInfo['comments']['artist'])             : '').'"';
				$SQLquery .= ', `tags` = "'.            mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']                          ) ? implode("\t", array_keys($ThisFileInfo['tags']))                          : '').'"';
				$SQLquery .= ', `encoder_options` = "'. mysqli_real_escape_string($link, trim((!empty($ThisFileInfo['audio']['encoder']) ? $ThisFileInfo['audio']['encoder'] : '').' '.(!empty($ThisFileInfo['audio']['encoder_options']) ? $ThisFileInfo['audio']['encoder_options'] : ''))).'"';
				$SQLquery .= ', `title` = "'.           mysqli_real_escape_string($link, $this_track_title).'"';
				$SQLquery .= ', `remix` = "'.           mysqli_real_escape_string($link, $this_track_remix).'"';
				$SQLquery .= ', `track` = "'.           mysqli_real_escape_string($link, $this_track_track).'"';
				$SQLquery .= 'WHERE (`filename` = "'. mysqli_real_escape_string($link, isset($ThisFileInfo['filenamepath']) ? $ThisFileInfo['filenamepath'] : '').'")';

			} elseif (!empty($_REQUEST['scan']) || !empty($_REQUEST['newscan'])) {

				//$SQLquery  = 'INSERT INTO `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'` (`filename`, `last_modified`, `md5_file`, `md5_data`, `md5_data_source`, `filesize`, `fileformat`, `audio_dataformat`, `video_dataformat`, `audio_bitrate`, `video_bitrate`, `playtime_seconds`, `artist`, `title`, `remix`, `album`, `genre`, `comment`, `track`, `comments_all`, `comments_id3v2`, `comments_ape`, `comments_lyrics3`, `comments_id3v1`, `warning`, `error`, `encoder_options`, `vbr_method`, `track_volume`) VALUES (';
				...
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['filenamepath']                  ) ?                          $ThisFileInfo['filenamepath']                    : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['file_modified_time']            ) ?                          $ThisFileInfo['file_modified_time']              : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['md5_file']                      ) ?                          $ThisFileInfo['md5_file']                        : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['md5_data']                      ) ?                          $ThisFileInfo['md5_data']                        : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['md5_data_source']               ) ?                          $ThisFileInfo['md5_data_source']                 : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['filesize']                      ) ?                          $ThisFileInfo['filesize']                        :  0).'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['fileformat']                    ) ?                          $ThisFileInfo['fileformat']                      : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['audio']['dataformat']           ) ?                          $ThisFileInfo['audio']['dataformat']             : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['video']['dataformat']           ) ?                          $ThisFileInfo['video']['dataformat']             : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['audio']['bitrate']              ) ?                 floatval($ThisFileInfo['audio']['bitrate'])               :  0).'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['video']['bitrate']              ) ?                 floatval($ThisFileInfo['video']['bitrate'])               :  0).'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['playtime_seconds']              ) ?                 floatval($ThisFileInfo['playtime_seconds'])               :  0).'", ';
				//$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']                          ) ?            implode("\t", $ThisFileInfo['tags'])                           : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']                          ) ? implode("\t", array_keys($ThisFileInfo['tags']))                          : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['comments']['artist']            ) ?            implode("\t", $ThisFileInfo['comments']['artist'])             : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, $this_track_title).'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, $this_track_remix).'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['comments']['album']             ) ?            implode("\t", $ThisFileInfo['comments']['album'])              : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['comments']['genre']             ) ?            implode("\t", $ThisFileInfo['comments']['genre'])              : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['comments']['comment']           ) ?            implode("\t", $ThisFileInfo['comments']['comment'])            : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, $this_track_track).'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['comments']                      ) ?                serialize($ThisFileInfo['comments'])                       : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']['id3v2']                 ) ?                serialize($ThisFileInfo['tags']['id3v2'])                  : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']['ape']                   ) ?                serialize($ThisFileInfo['tags']['ape'])                    : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']['lyrics3']               ) ?                serialize($ThisFileInfo['tags']['lyrics3'])                : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['tags']['id3v1']                 ) ?                serialize($ThisFileInfo['tags']['id3v1'])                  : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['warning']                       ) ?            implode("\t", $ThisFileInfo['warning'])                        : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['error']                         ) ?            implode("\t", $ThisFileInfo['error'])                          : '').'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, trim((!empty($ThisFileInfo['audio']['encoder']) ? $ThisFileInfo['audio']['encoder'] : '').' '.(!empty($ThisFileInfo['audio']['encoder_options']) ? $ThisFileInfo['audio']['encoder_options'] : ''))).'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['mpeg']['audio']['LAME']) ? 'LAME' : (!empty($ThisFileInfo['mpeg']['audio']['VBR_method']) ? $ThisFileInfo['mpeg']['audio']['VBR_method'] : '')).'", ';
				$SQLquery .= '"'.mysqli_real_escape_string($link, !empty($ThisFileInfo['replay_gain']['track']['volume']) ?                 floatval($ThisFileInfo['replay_gain']['track']['volume']) :  0).'")';
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		if (file_exists($row['filename'])) {
			...
			if (!empty($ThisFileInfo['replay_gain']['track']['volume'])) {
				...
				$SQLquery  = 'UPDATE `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`';
	...
	if (!empty($_REQUEST['m3u'])) {
		...
		$SQLquery .= ' FROM `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`';
		$SQLquery .= ' WHERE (`encoder_options` = "'.mysqli_real_escape_string($link, $_REQUEST['encodedbydistribution']).'")';
		$result = mysqli_query_safe($SQLquery);
		$NonBlankEncodedBy = '';
		$BlankEncodedBy = '';
		while ($row = mysqli_fetch_array($result)) {
	...
	if (isset($_REQUEST['showtagfiles'])) {
		$SQLquery  = 'SELECT `filename`, `encoder_options` FROM `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`';
		$SQLquery .= ' WHERE (`encoder_options` LIKE "'.mysqli_real_escape_string($link, $_REQUEST['showtagfiles']).'")';
		...
		$SQLquery  = 'SELECT `encoder_options`, COUNT(*) AS `num` FROM `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`';
		$SQLquery .= ' WHERE (`fileformat` NOT LIKE "'.implode('") AND (`fileformat` NOT LIKE "', $IgnoreNoTagFormats).'")';
	...
	while (($row = mysqli_fetch_array($result)) && ($row['num'] > 1)) {
		...
		$SQLquery .= ' FROM `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`';
		$SQLquery .= ' WHERE (`md5_data` = "'.mysqli_real_escape_string($link, $row['md5_data']).'")';
		$SQLquery .= ' ORDER BY `filename` ASC';
		$result2 = mysqli_query_safe($SQLquery);
	...
	if (isset($_REQUEST['m3uartist']) && isset($_REQUEST['m3utitle'])) {
		...
		$SQLquery  = 'SELECT `filename`';
		$SQLquery .= ' FROM `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`';
		$SQLquery .= ' WHERE (`artist` = "'.mysqli_real_escape_string($link, $_REQUEST['m3uartist']).'")';
		$SQLquery .= ' AND (`title` = "'.mysqli_real_escape_string($link, $_REQUEST['m3utitle']).'")';
	...
	if (!empty($_REQUEST['m3u'])) {
		...
		while (($row = mysqli_fetch_array($result)) && ($row['num'] > 1)) {
			$SQLquery  = 'SELECT `filename`';
			$SQLquery .= ' FROM `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`';
			$SQLquery .= ' WHERE (`artist` = "'.mysqli_real_escape_string($link, $row['artist']).'")';
			$SQLquery .= ' AND (`title` = "'.mysqli_real_escape_string($link, $row['title']).'")';
			if (!empty($_REQUEST['samemix'])) {
				$SQLquery .= ' AND (`remix` = "'.mysqli_real_escape_string($link, $row['remix']).'")';
			}
			$SQLquery .= ' ORDER BY `playtime_seconds` ASC, `remix` ASC, `filename` ASC';
			$result2 = mysqli_query_safe($SQLquery);
			while ($row2 = mysqli_fetch_array($result2)) {
	...
	$SQLquery .= ' FROM `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`';
	$SQLquery .= ' WHERE (`fileformat` = "'.mysqli_real_escape_string($link, $fileformat).'")';
	$SQLquery .= ' AND (`audio_dataformat` = "'.mysqli_real_escape_string($link, $audioformat).'")';
	$SQLquery .= ' ORDER BY `filename` ASC';
	$result = mysqli_query_safe($SQLquery);
	...
	if ($_REQUEST['vbrmethod'] == '1') {
		...
		$SQLquery  = 'SELECT `filename`';
		$SQLquery .= ' FROM `'.mysqli_real_escape_string($link, GETID3_DB_TABLE).'`';
		$SQLquery .= ' WHERE (`vbr_method` = "'.mysqli_real_escape_string($link, $_REQUEST['vbrmethod']).'")';
		$result = mysqli_query_safe($SQLquery);
